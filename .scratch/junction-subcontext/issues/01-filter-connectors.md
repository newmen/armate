...
- [ ] Rendering "Шахматы" and "Процесс" of `test/resources/demo.archimate` outputs no `Junction_Or(jc26, ...)`.
- [ ] Rendering "Семья" outputs `Junction_Or(jc26, ...)` plus its `Rel_Triggering` edges.
- [ ] `:certain` / `:certain+potential` modes do not reintroduce the Junction into an unplaced view.
- [ ] `merge_views` shows the Junction only when a placing view is merged.
- [ ] `related_elements` shows the Junction only when the induced sub-graph includes it.
- [ ] All existing MCP analytics/tools tests still pass (`clj-kondo` clean).
